...
<?php 
	while( $abierto_pedidos && $f = mysqli_fetch_assoc( $familias ) ){ 
		...
		if ( in_array( $f["idFamilia"], $familias_visibles ) ){
			$unidades = obtenerLimitePorCategoriaItem( $f["idFamilia"] );
?>
		...
		<table id="productos" align="center">
			...
			<?php 
				while( $item = mysqli_fetch_assoc( $items_familia ) ){ 
					...
					$sql = "SELECT * FROM Pedido as p, PedidoDetalle as d where p.idPedido = d.idPedido 
							and p.idColaborador = $idpersona and d.idItem = $id and (p.Fecha BETWEEN '$FirstDay' AND '$LastDay')";
